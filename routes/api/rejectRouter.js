...
var Reject = require('../../models/rejects');
                    ...
                    .then((requiredUser) => {
                        ...
                        if (!requiredBook) {
                            ...
                            Issue.find({
                                ...
                                .then((issues) => {
                                    notRejected = issues.filter((issue) => (!issue.accepted));
                                    if (notAccepted && notAccepted.length >= 3) {
                                        err = new Error(`The student has already issued 3 books. Please accept them first`);
                                        err.status = 400;
                                        return next(err);
                                    }
                                    else {
...
issueRouter.route('/:issueId')
    ...
    .put(cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
        Issue.findById(req.params.issueId)
                ...
                Books.findById(issue.book)
                    .then((requiredBook) => {
                        Issue.findByIdAndUpdate(req.params.issueId, {
                            ...
                            .catch((err) => res.status(400).json({ success: false, message: "Reject not Updated" }));
            ...
            .catch((err) => res.status(400).json({ success: false, message: "Reject not found" }))
    })
